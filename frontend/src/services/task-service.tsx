...
import { taskApiService, TaskFilters } from './api-service';
...
export const taskService = {
  ...
  async listTasks(filters?: TaskFilters): Promise<{ tasks: Task[]; total: number }> {
    try {
      return await taskApiService.listTasks(filters);
    } catch (error) {
  ...
  async completeTask(id: string): Promise<Task | null> {
    try {
      return await taskApiService.updateTask(id, { completed: true });
    } catch (error) {
